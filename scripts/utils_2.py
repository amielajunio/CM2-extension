...
def _is_int_like(x):
    # Split logic treats integer cardinalities with exact integer chunks.
    return isinstance(x, int) or (isinstance(x, float) and x.is_integer())


def _sp_cardinality(sp):
    # Predicate format: (entity, attr, num_values[, provenance_rel]).
    return sp[2] if len(sp) >= 3 else 1


def _split_predicate_value(sp, chunks):
    # Clone the same predicate preserving metadata and only changing value-count.
    split = []
    for c in chunks:
        s = list(sp)
        s[2] = c
...
def _split_value_with_cap(v, cap):
    # Split one predicate so that this predicate contributes at most "cap" values per batch.
    if cap <= 0:
...
def _split_value_unit_like(v):
    # Fallback when other predicates already saturate K: force near unit chunks on this predicate.
    if _is_int_like(v):
        iv = int(round(v))
        if iv <= 1:
            return [v]
...
def _expand_capped_sps(sps, K):
    ...
    # Iterative queue to avoid recursion and keep code easy to inspect/debug.
    queue = [list(sps)]
    batches = []
    ...
    while queue:
        ...
        # Split the currently largest expandable predicate first.
...
def costNumDocsKapped(domain,qp,K):
    # Assumption used by this model:
    # each AP may branch into multiple capped batches, then results are UNIONed
    # before executing the next AP (single join predicate propagation between APs).
    if K is None or K < 1:
        raise ValueError("K must be >= 1")
    ...
    for i in range(len(qp["aps"])):
        root = getRootOfColl(qp["aps"][i]["c"])
        sps = list(qp["aps"][i]["sp"])
        if i>0 and qp["type"] != "w": # aggiunto perche i piani di write non hanno sp
            sps.append(getJoinSp(domain,qp["aps"],i))
        qp["aps"][i]["sps"] = sps
        ...
        # Non-capped metrics (same semantics of costNumDocs)
        maxCardIx = math.prod([getCardOfAttr(domain,sp[0],sp[1]) for sp in isps])
        qp["aps"][i]["n_ix"] = math.prod([sp[2] for sp in sps]) * min(maxCardIx,maxCardInFilter(domain,sps)) / maxCardIx
        qp["aps"][i]["n_doc"] = filterInst(domain,qp["aps"][i]["c"],root,isps)
        qp["aps"][i]["n_res"] = filterInst(domain,qp["aps"][i]["c"],root,sps)
        qp["aps"][i]["n_q"] = 1

        # Capped metrics
        qp["aps"][i]["n_fv"] = math.prod([sp[2] for sp in sps])
        # If already under cap, avoid splitting and keep one batch.
        # Otherwise split into sub-queries that satisfy product(sp[2]) <= K.
        sps_batches = [list(sps)] if qp["aps"][i]["n_fv"] <= K else _expand_capped_sps(sps, K)
        qp["aps"][i]["n_batches"] = len(sps_batches)
        qp["aps"][i]["n_q_k"] = len(sps_batches)

        ap_n_ix_k = 0
        ap_n_doc_k = 0
        ap_n_res_k = 0

        for sps_batch in sps_batches:
            isps_batch = []
            for sp in sps_batch:
                for att in qp["aps"][i]["ix"]:
                    if att[0]==sp[0] and att[1]==sp[1]:
                        isps_batch.append(sp)

            maxCardIxBatch = math.prod([getCardOfAttr(domain,sp[0],sp[1]) for sp in isps_batch])
            n_ix_batch = math.prod([sp[2] for sp in sps_batch]) * min(maxCardIxBatch,maxCardInFilter(domain,sps_batch)) / maxCardIxBatch
            n_doc_batch = filterInst(domain,qp["aps"][i]["c"],root,isps_batch)
            n_res_batch = filterInst(domain,qp["aps"][i]["c"],root,sps_batch)

            ap_n_ix_k += n_ix_batch
            ap_n_doc_k += n_doc_batch
            ap_n_res_k += n_res_batch

        qp["aps"][i]["n_ix_k"] = ap_n_ix_k
        qp["aps"][i]["n_doc_k"] = ap_n_doc_k
        qp["aps"][i]["n_res_k"] = ap_n_res_k

    qp["n_ix"] = sum([qp["aps"][i]["n_ix"] for i in range(len(qp["aps"]))])
    qp["n_doc"] = sum([qp["aps"][i]["n_doc"] for i in range(len(qp["aps"]))])
    qp["n_res"] = sum([qp["aps"][i]["n_res"] for i in range(len(qp["aps"]))])
    qp["n_q"] = sum([qp["aps"][i]["n_q"] for i in range(len(qp["aps"]))])
    qp["n_ix_k"] = sum([qp["aps"][i]["n_ix_k"] for i in range(len(qp["aps"]))])
    qp["n_doc_k"] = sum([qp["aps"][i]["n_doc_k"] for i in range(len(qp["aps"]))])
    qp["n_res_k"] = sum([qp["aps"][i]["n_res_k"] for i in range(len(qp["aps"]))])
    qp["n_q_k"] = sum([qp["aps"][i]["n_q_k"] for i in range(len(qp["aps"]))])
    return
